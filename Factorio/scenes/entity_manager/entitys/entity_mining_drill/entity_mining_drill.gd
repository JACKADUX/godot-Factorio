...
func construct(_entity_manager):
	...
	_input_invantory.input("COAL", 100)
	
	_fuel_inventory = Inventory.new(1)
	_fuel_inventory.input("COAL", 100)
...
func _feed_fuel() -> bool:
	...
	slot.set_amount(slot.get_amount()-1)
...
func _start_work():
	var slots = _input_invantory.get_valid_slots()
	if not slots:
		_WSC.to_idle_state()
		return 
	_id = slots[0].get_id()
