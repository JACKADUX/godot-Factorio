...
@export var larg_grid := Vector2(320, 320)

## Virtual
func _ready():
	Globals.main_camera.view_changed.connect(queue_redraw)
	
func _draw():
	var area_rect = Globals.main_camera.get_view_rect()
	var zoom = Globals.main_camera.zoom.x
	if zoom > 0.5:
		draw_grid(area_rect, small_grid, Color(0.2,0.2,0.2), 2/zoom)
	draw_grid(area_rect, larg_grid, Color(0.1,0.1,0.1), 4/zoom)
